...
//	  }
	  ...
	  {   
		...
		public static int decompressWTO(final Inflater inflater, final byte[] buf, final int off, final int len) throws DataFormatException 
		  {   
		      ExecutorService executor = Executors.newSingleThreadExecutor();
		      Callable<Integer> callable = new Callable<Integer>() {
		          @Override
		          public Integer call() throws DataFormatException {
		              return inflater.inflate(buf,off,len);
		          }
		      };
		      Future<Integer> future = executor.submit(callable);
		      int count = 0;
