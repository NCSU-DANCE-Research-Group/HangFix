...
import cassandraIO.RandomAccessReader;
...
//    }
    ...
    public Scrubber(/*ColumnFamilyStore cfs,*/ SSTableReader sstable, /*boolean skipCorrupted, OutputHandler outputHandler, boolean isOffline,*/ 
    		File dataTmp) throws IOException
    {
//        this.cfs = cfs;
...
//                        : sstable.openDataReader(CompactionManager.instance.getRateLimiter());
        this.dataFile = RandomAccessReader.open(dataTmp);
//        this.indexFile = RandomAccessReader.open(new File(sstable.descriptor.filenameFor(Component.PRIMARY_INDEX)));
//        this.indexFile = RandomAccessReader.open(indexTmp);
...
//                    key = sstable.partitioner.decorateKey(ByteBufferUtil.readWithShortLength(dataFile));
                    //inject faults
                	key = sstable.partitioner.decorateKey(MockByteBufferUtil.readWithShortLength(dataFile));
                    if (/*sstable.descriptor.version.hasRowSizeAndColumnCount*/true)
                    {
...
//                        outputHandler.debug(String.format("row %s is %s bytes", ByteBufferUtil.bytesToHex(key.key), dataSize));
                ...
                {
                	System.out.println("inside loop, the dataFile.readLong is skipped");
                    throwIfFatal(th);
...
//                    nextIndexKey = indexFile.isEOF() ? null : ByteBufferUtil.readWithShortLength(indexFile);
                    nextIndexKey = null;//indexFile is EOF
//                    nextRowPositionFromIndex = indexFile.isEOF()
//                                             ? dataFile.length()
//                                             : RowIndexEntry.serializer.deserialize(indexFile, sstable.descriptor.version"ja").position;
                    nextRowPositionFromIndex = dataFile.length();//indexFile is EOF
                }
                catch (Throwable th)
                {
...
//                if (/*!sstable.descriptor.version.hasRowSizeAndColumnCount*/false)
//                {
...
//                    	//we don't care about this key, bc we assume the currentIndex==null
//                        try
//                        {
////                            SSTableIdentityIterator atoms = new SSTableIdentityIterator(sstable, dataFile, key, dataSize, true);
//                            if (prevKey != null && prevKey.compareTo(key) > 0)
