 ...
 public static final Log LOG = LogFactory.getLog(CombineFileInputFormat.class);
  ...
  public List<InputSplit> getSplits(JobContext job) 
    ...
    getMoreSplits(job, newpaths.toArray(new Path[newpaths.size()]), 
                  maxSize, minSizeNode, minSizeRack, splits);
    System.out.println("--after getMoreSplits function---");
  ...
  private void getMoreSplits(JobContext job, Path[] paths, 
    ...
    throws IOException {
	System.out.println("-------inside getMoreSplits function-------");
    Configuration conf = job.getConfiguration();
    ...
    int preSize = blockToNodes.size();
    // Process all racks over and over again until there is no more work to do.
    while (blockToNodes.size() > 0) {
    	LOG.info("Inside a while loop.");
    	try {
			Thread.sleep(5000);
			int curSize = blockToNodes.size();
			if(curSize == preSize) break;
  ...
  private static class OneFileInfo {
    ...
    OneFileInfo(Path path, Configuration conf,
      ...
      if (locations == null) {
        ...
        for (OneBlockInfo oneblock : blocks) {
          LOG.info("Inside OneFileInfo class------");
//          System.out.println("----inside OneFileInfo class------");
          // add this block to the block --> node locations map
//          if(oneblock.hosts.length != 0)//This is the patch we need to generated!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          blockToNodes.put(oneblock, oneblock.hosts);
