...
public class TestProcfsBasedProcessTree {
  ...
  protected static File TEST_ROOT_DIR = new File("/home/ting/DataLoopBugDetection/workspace_java/testInstrumentJavaClass/dataset",
      TestProcfsBasedProcessTree.class.getName() + "-localDir");

  private MockShellCommandExecutor shexec = null;
...
//        System.out.println(args.toString());
        shexec = new MockShellCommandExecutor(args.toArray(new String[0]));
        shexec.execute();
  ...
  private boolean createFail = false;
  ...
  private String getRogueTaskPID() {
    ...
    while (!f.exists()) {
      try {
    	  if(createFail) {LOG.info("pidFile creation failed"); break;}
    	  System.out.println("inside loop, pidFile does not exist.");
        Thread.sleep(500);
...
//    if(pid == null) {
//    	throw new IOException("pid is null...");
//    }
    ...
    System.out.println(lowestDescendant);
    //wait till lowest descendant process of Rougue Task starts execution
    while (!leaf.exists()) {
      try {
        Thread.sleep(500);
//        System.out.println("hanging here....");
      } catch (InterruptedException ie) {
        break;
      }
    }
