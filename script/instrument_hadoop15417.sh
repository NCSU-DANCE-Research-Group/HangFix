...
currentpath=$(pwd)
mkdir -p $currentpath/output_hadoop15417
rm -rf $currentpath/output_hadoop15417/*
...
libs=$libpath/guava-20.0.jar:$libpath/commons-lang-2.6.jar:$libpath/commons-logging-1.2.jar:$libpath/netty-3.7.0.Final-javadoc.jar:$libpath/netty-3.7.0.Final-sources.jar:$libpath/commons-io-2.6.jar:$libpath/netty-3.7.0.Final.jar:$libpath/commons-net-3.1.jar:$libpath/servlet-api-2.5.jar:$libpath/commons-collections-3.2.1.jar:$libpath/jackson-core-asl-1.9.13.jar:$libpath/guava-21.0.jar:$libpath/mockito-all-1.8.2.jar:$libpath/hadoop-annotations-0.23.0.jar:$libpath/jsr305-1.3.9.jar
cd $path
cp $currentpath/confFiles/conf_hadoop15417 $path/conf
java -cp $libs:$(pwd):$CLASSPATH typeDataContent.MainDriverScene hadoop15417.Buggycode hadoop15417.testcode
#this bug cannot be fixed by the bodytransform, it has to be fixed by scenetransform
#this is because the return of getActiveBody of inilizate() func from retrieveBlock() method is actually BafBody not JimpleBody. 